...
description: "An opinionated guide to installing R, RStudio, and the Tidyverse"
format:
  html:
...
::: {.callout-important}
Working with R and RStudio users within DPH my experience is the number one
impediment and the one I can least help you with is **NOT** having enough
permission or user rights to install the software.  If you are on a state
owned machine or centrally managed computer, please make sure you have the
ability to install software. In this tutorial you're going to see examples
and screenshots from my personal computers. I'll highlight important differences
across operating systems as they arise.
:::

If you have some new problem and, especially, the corresponding
solution, [I'd love to hear from
you!](mailto:charles.powell@ct.gov). These installation
instructions are brand new, bear with me it's the hardest part.
Another important note is that I love live interaction, so don't feel
you have to wait to ask questions.  Odds are if you have a question
someone else does as well.  Ask it.
...
This document is also very *opinionated* about the *best* way to
use R/RStudio.  There are plenty of other opinions so please
don't confuse my strong opinions with the *"one right way"* or the *"only
way"*.

If you feel up to doing it yourself.  [Here's one possible](https://rstudio-education.github.io/hopr/starting.html) 
simple set of documentation. As well as a host of very simple instructions
for using R and RStudio.
:::

For these sessions I'm going to start with the assumption that you don't
have a current version of the software installed.  It many cases you might be
better served by doing a fresh new install rather than other options. We'll address
that in a bit in @sec-startover for additional details.
